...
const staysInJayWorld = (...values: unknown[]) => {
  const copy = values.filter((value): value is string => typeof value === "string").join(" ");
  return !coreSolutionsPattern.test(copy) && !foreignLeakPattern.test(copy);
};
...
const fallbackFrames: Record<Tension, { reframe: string; pressure: string; human: string; close: string; notes: [string, string, string, string, string] }> = {
  ...
  routine: {
    ...
    human: "Hay rutinas que sigues cumpliendo incluso después de olvidar por qué empezaron.",
    ...
    notes: [
      ...
      "Cumplir una rutina puede sentirse correcto. Eso no demuestra que todavía proteja algo que valoras.",
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3 && !parsedWeek; attempt += 1) {
      const result = await generateText({
        ...
        system: `You are the editorial partner for JAY POST STUDIO. Write only in neutral Latin American Spanish using tú forms; never use voseo such as seguís, sentís, apagás or decís, and never insert an English word. ${jayEditorialWorld} ${jayClarityRule} ${jayReferenceVoice} ${jayEvidence} ${jayQualityGate} Build one complete five-post week. Every post must be an independent, complete idea; together they explore one theme from five genuinely different angles: first notice the behavior, then expose its hidden cost, challenge the convenient excuse, place it in ordinary life, and finish with a clear personal criterion. Do not write sequels, teasers, numbered parts or five paraphrases. Do not turn the theme into advice for professionals. Use ordinary human scenes: a Tuesday, a meal, a screen, a purchase, a conversation, a parent, a child, a quiet room, an avoided decision. Do not use clients, teams, companies or workplace examples. Every post title must state its exact claim in at least four words; never use role labels such as 'La implicación', 'La pregunta', 'El límite' or 'Lo normalizado' as titles. Graphic writing rules: SIMPLE uses one clear claim of 6–14 words and at most 85 characters. Text art uses a complete thought of 12–24 words and at most 145 characters. Context uses a concrete observation of 16–25 words and at most 155 characters. The carousel contains 4 or 5 complete steps of 7–13 words and at most 85 characters each. No graphic text may end in a connector such as 'que', 'de', 'con', 'porque' or 'y'. Never sacrifice grammar or meaning to meet a limit and never expect the app to truncate your writing. Captions must contain exactly two paragraphs and 70–120 words total. Write 4–6 complete sentences per caption: paragraph one develops a recognizable observation or scene; paragraph two explains the tension or consequence. Captions clarify the graphic instead of repeating it. Before returning JSON, count the words in every caption and verify every character limit. Return only a valid JSON object with title, thesis, arc and exactly 5 posts in the supplied order. Every post needs title, label, copy, caption and pillar. Only the carousel post gets slides. Omit slides from every other post. counterpoint is optional and only useful for a split composition.`,
        prompt: `Tema semanal: ${topic}\nNombre del tema: ${title || "Semana JAY"}\nTensión: ${tension}\nLos cinco objetos de posts deben respetar exactamente este orden: ${weeklyPlans.map((plan, index) => `${index + 1}) day=${plan.day}, role=${plan.role}, format=${plan.format}, templateId=${plan.templateId}, ${plan.format === "carousel" ? "slides obligatorio con 4 o 5 textos completos" : "sin slides"}`).join(" | ")}\nObjetivo: crear ideas personales claras, memorables y comprensibles en la voz de JAY.${attempt ? ` La respuesta anterior fue rechazada. Repárala conservando sus buenas ideas y corrigiendo cada incumplimiento. Expande TODOS los captions a 80–100 palabras, exactamente dos párrafos y 4–6 oraciones. Revisa también: títulos específicos de 4 palabras o más; copy dentro del máximo; ningún texto terminado en conector; slides solo en el carrusel; cinco ideas distintas; cero lenguaje profesional; español neutro sin voseo. Devuelve únicamente el JSON reparado. RESPUESTA ANTERIOR:\n${text.slice(0, 14000)}` : ""}`,
      });
      text = result.text;
